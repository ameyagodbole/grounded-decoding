...
class LlamaHeadMaskDecoderLayer(nn.Module):
...
class LlamaHeadMaskModel(LlamaModel):
    """
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [LlamaHeadMaskDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class LlamaHeadMaskForCausalLM(LlamaForCausalLM):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = LlamaHeadMaskModel(config)
